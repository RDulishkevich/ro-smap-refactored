        ...
        const cards = list.length
            ? list.map((e) => {
                ...
                <button type="button" class="event-card" onclick="window.openEventDetail('${esc(e.id)}')">
                    ${e.pinned ? '<span class="event-card__pin"><i class="icon-location-tick"></i></span>' : ''}
                    ...
                    <p class="event-card__meta"><i class="icon-calendar"></i> ${esc(window.formatEventRange(e))}</p>
                    <p class="event-card__meta"><i class="icon-people"></i> ${going} ${window.pluralRu ? window.pluralRu(going, 'участник', 'участника', 'участников') : (going === 1 ? 'участник' : 'участников')}</p>
                </button>`;
            }).join('')
            : `<div class="events-empty"><i class="icon-calendar"></i><p>Пока нет ивентов</p><p class="text-[11px] opacity-70">Админ может создать конкурс, встречу или челлендж</p></div>`;

        bodyHtml = `
            <div class="events-tabs">${tabs}</div>
            ${isAdmin ? `<button type="button" class="events-create-btn" onclick="window.openEventEditor()"><i class="icon-add"></i> Создать ивент</button>` : ''}
...
window.renderEventDetail = function(e) {
    ...
    const adminBar = isAdmin
        ? `<div class="event-admin-bar">
            <button type="button" onclick="window.openEventEditor('${e.id}')"><i class="icon-edit-2"></i></button>
            <button type="button" onclick="window.toggleEventPin('${e.id}')"><i class="icon-location-tick"></i></button>
            ...
            <button type="button" onclick="window.deleteEvent('${e.id}')" class="text-red-500"><i class="icon-trash"></i></button>
           </div>`
        : '';

    return `
        <button type="button" class="events-back" onclick="window.backToEventsList()"><i class="icon-arrow-left"></i> К списку</button>
        ...
        <div class="event-detail">
            ...
            ${e.theme ? `<p class="event-card__theme">${esc(e.theme)}</p>` : ''}
            <p class="event-card__meta mt-2"><i class="icon-calendar"></i> ${esc(window.formatEventRange(e))}</p>
            ${e.place?.label ? `<p class="event-card__meta"><i class="icon-location"></i> ${esc(e.place.label)}</p>` : ''}
        ...
        prizesEl.innerHTML = (window.__eventEditorPrizes || []).map((p, i) => {
            ...
            <div class="event-editor-prize">
                <div class="event-editor-row">
                    ...
                    <button type="button" onclick="window.__eventEditorPrizes.splice(${i},1);window.renderEventEditorLists()"><i class="icon-close-circle"></i></button>
                </div>
    ...
    if (condEl) {
        condEl.innerHTML = (window.__eventEditorConditions || []).map((c, i) => `
            <div class="event-editor-row">
                ...
                <button type="button" onclick="window.__eventEditorConditions.splice(${i},1);window.renderEventEditorLists()"><i class="icon-close-circle"></i></button>
            </div>`).join('');
    }
};
