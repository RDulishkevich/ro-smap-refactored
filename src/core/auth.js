export function initAuth() {
    ...
    window.logoutUser = async function() {
        const confirmed = await window.CustomUI.open({
            title: '<i class="icon-logout mr-2 text-red-500"></i>Выйти из аккаунта?',
    ...
    window.renderSessionsPanel = function() {
        ...
        if (!sessions.length) {
            container.innerHTML = `<div class="text-center py-12 text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm"><i class="icon-routing text-4xl mb-3 opacity-30 block"></i><p class="font-medium text-sm">У вас пока нет экспедиций.</p><p class="text-xs mt-1">Создайте сессию, чтобы группировать записи одного выезда перед публикацией.</p></div>`;
            return;
        }
                ...
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-[11px] text-slate-400 mt-0.5 flex items-center gap-1 flex-wrap">
                            ${createdDate ? `<span><i class="icon-calendar mr-1"></i>${createdDate}</span> · ` : ''}
                            <span>${soundsInSession.length} ${soundsInSession.length === 1 ? 'запись' : 'записей'}</span>
                        </p>
                        ${session.route ? `<p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1"><i class="icon-routing mr-1 opacity-60"></i>${session.route}</p>` : ''}
                        ${session.purpose ? `<p class="text-xs text-slate-500 dark:text-slate-400 mt-1">${session.purpose}</p>` : ''}
                        ${participantNames.length ? `<p class="text-[11px] text-slate-400 mt-1"><i class="icon-people mr-1 opacity-60"></i>${participantNames.join(', ')}</p>` : ''}
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-2">
                            ${(session.videoLinks || []).map(url => `<a href="${url}" target="_blank" rel="noopener" class="profile-link-chip" onclick="event.stopPropagation()"><i class="icon-video"></i>Видео</a>`).join('')}
                            ${(session.links || []).map(url => `<a href="${url}" target="_blank" rel="noopener" class="profile-link-chip" onclick="event.stopPropagation()"><i class="icon-link-2"></i>Ресурс</a>`).join('')}
                        </div>` : ''}
                    </div>
    ...
    window.renderSidebarExpeditions = function() {
        ...
        const resetChip = `
            <button onclick="window.setSidebarSessionFilter(null)" class="sidebar-expedition-reset ${!window.activeSessionId ? 'active' : ''}">
                <i class="icon-document-text"></i>Все звуки
            </button>`;
        ...
        container.innerHTML = resetChip + sessions.map(session => {
            ...
            const thumb = (session.photos && session.photos[0])
                ...
                : `<span class="sidebar-expedition-thumb sidebar-expedition-thumb-fallback"><i class="icon-routing"></i></span>`;
            return `
                ...
                <div class="flex items-start gap-2.5">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-[10px] text-slate-400 mt-0.5 truncate">
                            <i class="icon-profile-circle mr-1 opacity-60"></i>${session.ownerName}
            ...
            ? `<button onclick="window.openSessionModal()" class="sidebar-expedition-add"><i class="icon-add mr-1.5"></i>Новая экспедиция</button>`
            : '');
    };
    ...
    window.renderSessionPhotosPreview = function() {
        ...
        container.innerHTML = (window.__sessionFormPhotos || []).map((src, i) => `
            <div class="relative">
                ...
                <button type="button" onclick="window.removeSessionPhoto(${i})" class="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center text-[10px] shadow-md"><i class="icon-close-circle"></i></button>
            </div>
        `).join('');
    };
    ...
    window.logoutAllDevices = async function() {
        const confirmed = await window.CustomUI.open({
            title: '<i class="icon-monitor mr-2 text-red-500"></i>Выйти везде?',
    ...
    window.renderCabinet = function() {
        ...
        if (badgeEl && window.getMyProgress && window.getLevelTitle) {
            ...
            badgeEl.innerHTML = `<i class="icon-tick-circle"></i>${window.getLevelTitle(prog.level)}`;
        }
        ...
        if (joinedEl) joinedEl.innerHTML = `<i class="icon-calendar"></i>${joinedDate && !isNaN(joinedDate) ? joinedDate.toLocaleDateString('ru-RU') : new Date().toLocaleDateString('ru-RU')}`;

        const mySounds = window.getUserSounds(login, window.currentUser.username, { includeAllStatuses: true });

        const statsEl = document.getElementById('cabinet-user-stats');
        if (statsEl) statsEl.innerHTML = `<i class="icon-microphone"></i>${mySounds.length} звуков`;
        ...
        if(mySounds.length === 0) {
            ...
            list.innerHTML = `<div class="text-center py-12 text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm"><i class="icon-microphone-slash text-4xl mb-3 opacity-30 block"></i><p class="font-medium text-sm">Вы еще не загрузили ни одного звука.</p><p class="text-xs mt-1">Опубликованные вами звуки появятся здесь.</p></div>`;
            return;
        }
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow gap-3">
                <div class="flex items-center gap-3 overflow-hidden">
                    ...
                    <div class="flex-1 min-w-0 pr-2">
                        ...
                        ${isRejectedLike(s) && s.rejectionReason ? `<p class="text-[11px] text-red-600 dark:text-red-400 mt-1 leading-snug"><i class="icon-danger mr-1"></i>${s.rejectionReason}</p>` : ''}
                ...
                <div class="flex items-center gap-2 shrink-0 sm:pr-2">
                    ...
                    <button onclick="window.editSound('${s.id}'); window.closeCabinet();" class="flex-1 sm:flex-none sm:w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 flex items-center justify-center transition-colors shadow-sm font-bold text-xs" title="Редактировать">
                        <i class="icon-edit-2 sm:mr-0 mr-1"></i> <span class="sm:hidden">Изменить</span>
                    </button>`}
                    <button onclick="window.deleteSoundFromCloud('${s.id}')" class="flex-1 sm:flex-none sm:w-9 h-9 rounded-xl bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 flex items-center justify-center transition-colors shadow-sm font-bold text-xs" title="Удалить">
                        <i class="icon-trash sm:mr-0 mr-1"></i> <span class="sm:hidden">Удалить</span>
                    </button>
                </div>
            </div>
        `}).join('');
    ...
    window.renderAdminList = function() {
        ...
        if (!sounds.length) {
            ...
            list.innerHTML = `<div class="text-center py-10 text-slate-400"><i class="icon-clipboard-tick text-3xl mb-2 opacity-30 block"></i><p class="text-sm font-medium">${emptyMsg}</p></div>`;
            if (window.refreshAdminRailBadge) window.refreshAdminRailBadge();
            return;
        }
            ...
            return `
                <div class="admin-entity-row">
                    <button type="button" class="admin-entity-main min-w-0 flex-1 text-left" onclick="window.openedFromAdmin=true; window.closeCabinet(); window.selectSound('${s.id}'); window.openDetailsModal();">
                        ...
                        ${(status === 'rejected' || returned) && s.rejectionReason ? `<p class="text-[11px] text-red-500 mt-0.5 line-clamp-2"><i class="icon-danger mr-1"></i>${s.rejectionReason}</p>` : ''}
                    </button>
                    <button type="button" onclick="event.stopPropagation(); window.openAdminSoundActions('${s.id}', event)" class="admin-actions-btn shrink-0"><i class="icon-more"></i> Действия</button>
                </div>
            `;
        }).join('');
        if (window.refreshAdminRailBadge) window.refreshAdminRailBadge();
    ...
    window.renderAdminExpeditionsList = function() {
        ...
        el.innerHTML = list.map((s) => {
            ...
            return `<div class="admin-entity-row">
                ...
                <button type="button" class="admin-actions-btn shrink-0" onclick="event.stopPropagation(); window.openAdminExpeditionActions('${esc(s.id)}', event)"><i class="icon-more"></i> Действия</button>
            </div>`;
        }).join('');
    };
    ...
    window.getAdminSoundActionItems = function(soundId) {
        ...
        const status = s.status || 'published';
        const items = [
            { icon: 'icon-eye', label: 'Просмотреть', tone: 'primary', onClick: () => { window.openedFromAdmin = true; window.closeCabinet(); window.selectSound(soundId); window.openDetailsModal(); } },
            { icon: 'icon-edit-2', label: 'Изменить', tone: 'primary', onClick: () => { window.openedFromAdmin = true; window.editSound(soundId); window.closeCabinet(); } }
        ];
        if (status === 'pending') {
            items.push({ icon: 'icon-tick-circle', label: 'Одобрить', tone: 'success', onClick: () => window.setSoundStatus(soundId, 'published') });
            items.push({ icon: 'icon-close-circle', label: 'Отклонить', tone: 'danger', onClick: () => window.setSoundStatus(soundId, 'rejected') });
        } else {
            if (status !== 'published') items.push({ icon: 'icon-tick-circle', label: 'Опубликовать', tone: 'success', onClick: () => window.setSoundStatus(soundId, 'published') });
            if (status !== 'pending') items.push({ icon: 'icon-clock', label: 'На модерацию', tone: 'warning', onClick: () => window.setSoundStatus(soundId, 'pending') });
            if (status !== 'rejected') items.push({ icon: 'icon-forbidden', label: 'Отклонить', tone: 'danger', onClick: () => window.setSoundStatus(soundId, 'rejected') });
        }
        if (window.isCurrentUserAdmin && window.isCurrentUserAdmin()) {
            items.push({ icon: 'icon-trash', label: 'Удалить', tone: 'danger', onClick: () => window.deleteSoundFromCloud(soundId) });
        }
        return items;
    };
    ...
    window.openDetailsAdminActions = function(ev) {
        ...
        const status = s.status || 'published';
        const items = [
            { icon: 'icon-edit-2', label: 'Изменить', tone: 'primary', onClick: () => { window.closeDetailsModal(); window.editSound(id); } }
        ];
        if (status === 'pending') {
            items.push(
                { icon: 'icon-tick-circle', label: 'Одобрить', tone: 'success', onClick: () => window.setSoundStatus(id, 'published') },
                { icon: 'icon-close-circle', label: 'Отклонить', tone: 'danger', onClick: () => window.setSoundStatus(id, 'rejected') }
            );
        } else if (status === 'rejected') {
            items.push(
                { icon: 'icon-tick-circle', label: 'Опубликовать', tone: 'success', onClick: () => window.setSoundStatus(id, 'published') },
                { icon: 'icon-clock', label: 'Вернуть на модерацию', tone: 'warning', onClick: () => window.setSoundStatus(id, 'pending') }
            );
        } else if (status === 'published') {
            items.push(
                { icon: 'icon-clock', label: 'Вернуть на модерацию', tone: 'warning', onClick: () => window.setSoundStatus(id, 'pending') },
                { icon: 'icon-forbidden', label: 'Отклонить', tone: 'danger', onClick: () => window.setSoundStatus(id, 'rejected') }
            );
        } else {
            items.push(
                { icon: 'icon-tick-circle', label: 'Опубликовать', tone: 'success', onClick: () => window.setSoundStatus(id, 'published') },
                { icon: 'icon-clock', label: 'На модерацию', tone: 'warning', onClick: () => window.setSoundStatus(id, 'pending') }
            );
        }
        if (window.isCurrentUserAdmin && window.isCurrentUserAdmin()) {
            items.push({ icon: 'icon-trash', label: 'Удалить', tone: 'danger', onClick: () => window.deleteSoundFromCloud(id) });
        }
        ...
        container.innerHTML = reports.map(r => {
            ...
            <div class="admin-entity-row ${r.status === 'resolved' ? 'is-muted' : ''}">
                <button type="button" class="admin-entity-main min-w-0 flex-1 text-left" onclick="window.openReportDetail('${r.soundId}', '${r.id}')">
                    ...
                    <p class="text-[11px] text-red-500 dark:text-red-400 mt-1 line-clamp-2"><i class="icon-quote-down mr-1 opacity-60"></i>${reasonShort}</p>
                </button>
                <button type="button" onclick="event.stopPropagation(); window.openAdminReportActions('${r.soundId}', '${r.id}', event)" class="admin-actions-btn shrink-0"><i class="icon-more"></i> Действия</button>
            </div>`;
        }).join('');
    };
    ...
    window.openReportDetail = function(soundId, reportId) {
        ...
        if (title) title.innerHTML = `<i class="icon-flag mr-2 text-red-500"></i>Жалоба № ${r.number || '–'}`;
        let targetText = '';
        if (r.type === 'comment') {
    ...
    window.openAdminReportActions = function(soundId, reportId, ev) {
        ...
        if (!r) return;
        const items = [
            { icon: 'icon-eye', label: 'Открыть текст жалобы', tone: 'primary', onClick: () => window.openReportDetail(soundId, reportId) },
            { icon: 'icon-musicnote', label: 'Просмотреть запись', tone: 'primary', onClick: () => { window.closeReportDetailModal(); window.openedFromAdmin = true; window.closeCabinet(); window.selectSound(soundId); window.openDetailsModal(); } }
        ];
        if (r.type === 'comment' && s && (s.comments || []).some(c => c.id === r.commentId)) {
            items.push({ icon: 'icon-trash', label: 'Удалить комментарий', tone: 'danger', onClick: () => window.deleteReportedComment(soundId, r.commentId, reportId) });
        }
        if (r.status !== 'resolved') {
            items.push({ icon: 'icon-tick-circle', label: 'Отметить решённой', tone: 'success', onClick: () => window.resolveReport(soundId, reportId) });
        }
    ...
    window.renderAdminUsersList = function() {
        ...
        el.innerHTML = rows.map(p => {
            ...
            <div class="admin-entity-row ${isBlocked ? 'is-muted' : ''}">
                <button type="button" class="admin-entity-main min-w-0 flex-1 text-left" onclick="window.openPublicProfile('${safeLogin}', '${safeName}')">
                    <div class="admin-user-row-name flex items-center gap-2">
                        ${p.avatar
                            ...
                            : `<span class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center shrink-0"><i class="icon-profile-circle opacity-60 text-sm"></i></span>`}
                ...
                <button type="button" onclick="window.openAdminUserActions('${safeLogin}', event)" class="admin-actions-btn shrink-0"><i class="icon-more"></i> Действия</button>
            </div>`;
        }).join('');
    };
    ...
    window.renderAdminSupportList = function() {
        ...
        if (!filteredRows.length) {
            el.innerHTML = `<div class="text-center py-8 px-3">
                <i class="icon-headphone text-2xl text-slate-300 dark:text-slate-600 mb-3 block"></i>
    ...
    window.openAdminUserActions = function(login, ev) {
        ...
        const items = [
            { icon: 'icon-profile-circle', label: 'Открыть профиль', tone: 'primary', onClick: () => window.openPublicProfile(login, p.displayName || login) },
            { icon: 'icon-chart-2', label: 'Сводка', tone: 'primary', onClick: () => window.openUserActivityModal(login) }
        ];
        if (window.isCurrentUserStaff && window.isCurrentUserStaff()) {
            items.push({
                icon: 'icon-sms',
        ...
        if (canManageRoles) {
            items.splice(1, 0, { icon: 'icon-medal', label: 'Звания', tone: 'warning', onClick: () => window.openBadgeAssignModal(login) });
        }
        if (canManageRoles && login !== 'admin') {
            if (role !== 'admin') {
                items.push({ icon: 'icon-user', label: 'Сделать администратором', tone: 'primary', onClick: () => window.setUserStaffRole(login, 'admin') });
            }
            if (role !== 'moderator') {
                items.push({ icon: 'icon-user-tick', label: 'Сделать модератором', tone: 'primary', onClick: () => window.setUserStaffRole(login, 'moderator') });
            }
            if (role !== 'user') {
                items.push({ icon: 'icon-user', label: 'Сделать пользователем', tone: 'warning', onClick: () => window.setUserStaffRole(login, 'user') });
            }
            items.push({ icon: 'icon-forbidden', label: isBlocked ? 'Разблокировать' : 'Заблокировать', tone: isBlocked ? 'success' : 'danger', onClick: () => window.setUserBlocked(login, !isBlocked) });
        }
    ...
    window.setUserBlocked = async function(login, blocked) {
        ...
        if (idx < 0) return;
        const confirmed = await window.CustomUI.open({
            title: blocked ? '<i class="icon-forbidden mr-2 text-red-500"></i>Заблокировать?' : 'Разблокировать?',
            ...
            <div>
                ...
                ${data.events.length ? data.events.map(e => {
                    ...
                    <div class="activity-row ${e.soundId ? 'cursor-pointer' : ''}" ${clickable}>
                        <i class="${icons[e.type] || 'icon-info-circle text-slate-400'} shrink-0 mt-0.5"></i>
                        <div class="min-w-0 flex-1">
    ...
    window.renderNotificationsList = function() {
        ...
        const icons = {
            ...
            message: 'icon-sms',
            badge: 'icon-medal',
            expedition: 'icon-routing',
            moderation: 'icon-clipboard-tick'
        };
        ...
        list.innerHTML = items.map(n => `
            <button onclick="window.openNotification('${esc(n.id)}')" class="notif-item ${n.read ? '' : 'unread'} w-full text-left">
                <i class="${icons[n.type] || 'icon-notification'} notif-item-icon"></i>
                <div class="min-w-0 flex-1">
    ...
    window.showMessagesConversations = function() {
        ...
        conv.innerHTML = rows.map(([peer, last]) => {
            ...
            const ticks = last._outgoingHint
                ? `<span class="msg-ticks msg-ticks--list ${last.read ? 'is-read' : 'is-delivered'}" title="${last.read ? 'Просмотрено' : 'Доставлено'}"><i class="${last.read ? 'icon-tick-circle' : 'icon-tick-circle'}"></i></span>`
                : '';
    ...
    window.updateThreadPeerHeader = function(peerLogin) {
        ...
        if (isSupport) {
            ...
            if (fallback) {
                ...
                fallback.innerHTML = '<i class="icon-headphone"></i>';
            }
            ...
            if (fallback) {
                fallback.classList.remove('hidden');
                fallback.innerHTML = '<i class="icon-user"></i>';
            }
        }
    };
    ...
    window.renderMessageTicks = function(m) {
        ...
        const icon = read ? 'icon-tick-circle' : 'icon-tick-circle';
    ...
    window.openMessageMenu = function(msgId, position) {
        ...
        if (!m.deleted) {
            items.push({ icon: 'icon-undo', label: 'Ответить', onClick: () => window.startMessageReply(msgId) });
            items.push({
            ...
            if (isMine) {
                items.push({ icon: 'icon-edit-2', label: 'Редактировать', onClick: () => window.editMessage(msgId) });
                items.push({ icon: 'icon-trash', label: 'Удалить', danger: true, onClick: () => window.deleteMessage(msgId) });
            } else {
                items.push({ icon: 'icon-flag', label: 'Пожаловаться', danger: true, onClick: () => window.reportMessage(msgId) });
            }
        }
        if (m.fromId) {
    ...
    window.deleteMessage = async function(msgId) {
        ...
        if (found.msg.fromId !== myLogin) return;
        const confirmed = await window.CustomUI.open({
            title: '<i class="icon-trash mr-2 text-red-500"></i>Удалить сообщение?',
    ...
    window.reportMessage = async function(msgId) {
        ...
        const reason = await window.CustomUI.open({
            title: '<i class="icon-flag mr-2 text-red-500"></i>Пожаловаться на сообщение',
    ...
    const ECO_ICONS = { biophony: 'icon-tree', geophony: 'icon-dropbox', anthrophony: 'icon-building' };
    ...
    window.renderMyAnalytics = function() {
        ...
        if (!data || data.totalPublished === 0) {
            container.innerHTML = `<div class="text-center py-12 text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm"><i class="icon-chart text-4xl mb-3 opacity-30 block"></i><p class="font-medium text-sm">Пока нет данных для аналитики.</p><p class="text-xs mt-1">Она появится, когда ваши опубликованные записи начнут слушать и скачивать.</p></div>`;
            return;
        }
                ...
                <div class="space-y-2">
                    ${data.topSounds.map((s, i) => `
                        <div class="flex items-center gap-2.5 py-1.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/60 rounded-lg px-1.5 -mx-1.5 transition-colors" onclick="window.selectSound('${s.id}'); window.closeCabinet();">
                            ...
                            <span class="text-[10px] text-slate-400 shrink-0"><i class="icon-headphone mr-0.5"></i>${s.plays || 0}</span>
                            <span class="text-[10px] text-slate-400 shrink-0"><i class="icon-document-download mr-0.5"></i>${s.downloads || 0}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
